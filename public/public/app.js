...
document.addEventListener('DOMContentLoaded', async () => {
    // Hidrata ícones SVG em toda a página
    hydrateIcons();
    ...
    // Onboarding: aviso de uso responsável (aparece 1x)
    checkOnboarding();
...
function hydrateIcons() {
    ...
    // Hydrate all elements with data-icon attribute
    document.querySelectorAll('[data-icon]').forEach(el => {
        ...
        if (svg) {
            ...
            // Set default size based on element class
            ...
            } else if (el.classList.contains('btn-icon')) {
                el.style.width = '16px';
                el.style.height = '16px';
            } else if (el.classList.contains('arrow-icon')) {
                el.style.width = '14px';
                el.style.height = '14px';
                el.style.transition = 'transform var(--transition-fast, 150ms ease-out)';
            } else if (el.classList.contains('trial-icon')) {
                el.style.width = '16px';
                el.style.height = '16px';
            } else if (el.classList.contains('alert-icon')) {
                el.style.width = '24px';
    ...
    if (logoEl && ICONS['message-circle']) {
        logoEl.innerHTML = ICONS['message-circle'];
    }
    // Logout button
    const logoutBtn = document.getElementById('btn-logout');
    if (logoutBtn && ICONS['log-out']) {
        logoutBtn.innerHTML = `<span style="display:inline-flex;width:18px;height:18px;">${ICONS['log-out']}</span>`;
    }
    // Mobile menu button
    const mobileBtn = document.getElementById('mobile-menu-btn');
    if (mobileBtn && ICONS['menu']) {
        mobileBtn.innerHTML = `<span style="display:inline-flex;width:22px;height:22px;color:var(--text);">${ICONS['menu']}</span>`;
    }
}
...
function updateGreeting() {
    ...
    const name = CURRENT_USER.name ? CURRENT_USER.name.split(' ')[0] : '';
...
function renderTrialBanner() {
    ...
    if (CURRENT_USER.plan === 'unlimited' || CURRENT_USER.role === 'admin') {
        textEl.textContent = 'Admin — Acesso Ilimitado';
        const banner = document.getElementById('trial-banner');
        if (banner) banner.style.background = 'rgba(37, 211, 102, 0.06)';
        return;
    }
    ...
    if (CURRENT_USER.plan === 'trial') {
        ...
        if (diffDays <= 0) {
            textEl.textContent = 'Seu teste expirou — escolha um plano para continuar';
            textEl.style.color = 'var(--danger)';
        } else {
            textEl.textContent = `Teste Grátis · ${diffDays} dia${diffDays > 1 ? 's' : ''} restante${diffDays > 1 ? 's' : ''}`;
            // Show progress bar
            if (progressEl && progressBar) {
                progressEl.style.display = 'block';
                progressBar.style.width = `${(diffDays / 7) * 100}%`;
                if (diffDays <= 2) progressBar.style.background = 'var(--danger)';
            }
        }
    } else {
        textEl.textContent = `${planName} · ${maxG} grupos · ${maxS} agendamentos · ${maxI} WA`;
    }
}
...
function filterDashboard(filter, btn) {
    dashFilter = filter;
    document.querySelectorAll('.tab-modern, .tab').forEach(t => t.classList.remove('active'));
    btn.classList.add('active');
    renderSchedules();
}
